...
public static class ResponsiveImageHelper
{
    ...
    /// 宽度来源：向上遍历视觉树，取最近的具有实际宽度（ActualWidth &gt; 0）的祖先。
    /// </summary>
    ...
    public static void MakeWidthFollowAncestor(FrameworkElement target, double fraction, bool applyAsMax = true)
    {
        double lastWidth = double.NaN;

        void Apply(FrameworkElement source)
        {
            if (source.ActualWidth <= 0) return;
            var w = source.ActualWidth * fraction;
        ...
        target.Loaded += (_, _) =>
        {
            // 向上遍历视觉树：取最近的具有实际宽度（ActualWidth > 0）的祖先作为宽度来源。
            FrameworkElement? source = null;
            FrameworkElement? fallback = null;
            var p = VisualTreeHelper.GetParent(target) as FrameworkElement;
            while (p != null)
            {
                if (p.ActualWidth > 0)
                {
            ...
            // 若 Loaded 时父级宽度尚未就绪（ActualWidth 仍为 0），延迟重试一次
            if (source.ActualWidth <= 0)
            {
                UIThread.Post(() => Apply(source));
            }
        };
    }
    ...
    public static void MakeInlineWidthFollowTextBlock(FrameworkElement reference, FrameworkElement image, string widthSpec, double defaultFraction)
    {
        ...
        void Apply()
        {
            if (textBlock == null || textBlock.ActualWidth <= 0) return;
            double viewportWidth = textBlock.ActualWidth;
            double w;
        ...
        reference.Loaded += (_, _) =>
        {
            var p = VisualTreeHelper.GetParent(reference) as FrameworkElement;
            while (p != null)
            {
