...
public class CountdownViewModel : INotifyPropertyChanged, IDisposable
{
    ...
    {
        if (e.PropertyName == nameof(CountdownSettings.Text1) ||
            e.PropertyName == nameof(CountdownSettings.Text2) ||
        ...
        if (e.PropertyName == nameof(CountdownSettings.TimeFormat))
        {
            UpdateCountdown();
            UpdateRefreshMode();
        }
    ...
    private CountdownDisplayData ProcessCountdownInternal(DateTime now)
    {
        ...
        {
            // 全部倒计时已结束：显示「{组件内所有倒计时的名称（空格分隔）} 已结束」（去掉原固定前缀"倒计时"）。
    ...
    {
        return OptimizedTimeFormatter.FormatTime(format, secondsLeft, millisecondsLeft, now, targetDate, startTime, targetTime, enableTimeCorrection);
    }
